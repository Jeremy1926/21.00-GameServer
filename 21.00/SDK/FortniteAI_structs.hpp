...
struct FBotCosmeticTableRowsInfo
{
	void* VTable;
	float TotalWeight;
	class FName SourceDataTableName;
	struct FGameplayTagContainer ExcludedCosmeticTags;
};

struct alignas(0x8) FBotCosmeticItemTableRowsInfo : FBotCosmeticTableRowsInfo
{
	TArray<struct FFortBotCosmeticItemDataTableRow const*> FilteredDataTableRows;
	TMap<struct FGameplayTag, struct FPrimaryAssetId> ItemsForSetTags;
	TMap<struct FPrimaryAssetId, struct FGameplayTagContainer> ExclusiveTagsForItems;
	bool bIsOptionalItem;
	uint8 Pad[0x7];
};
...
public:
	...
	uint8                                         Pad_0[0x4];                                       // 0x0000(0x0028)(Fixing Struct Size After Last Property [ Dumper-7 ])
	TSubclassOf<class UFortAthenaAISpawnerData> AISpawnerData;
	TArray<TDelegate<void(class AActor*, int)>> SpawnedCallbacks;
};
...
public:
	...
	int32 TotalSpawningInfoWeight;
	int32 CachedTotalAmountToSpawnComputed;
};
